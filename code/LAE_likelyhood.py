...
ncatRR=1000
fc=0.22 #Contamination fraction. 0 for no contamination.

Acor=1.0/(1-fc)
Acor=Acor*Acor # Amplutide correction factor of the ACF
...
def rejectionsample(arraySize,sampleSize,randvar,func):
#taken from stackoverflow forum: how do i create a LIST of unique random numbers
# This function takes a random subsample of length SampleSize from an sample of lenght arraySize 
    answer = set()
    answerSize = 0
    ...
    while answerSize < sampleSize:
        r = np.random.randint(0,arraySize)
        p=np.random.rand()
        if func(randvar[r])<p:continue
        if r not in answer: answerSize += 1;answer.add(r)
    answer=np.array([int(x) for x in answer])
    return answer
...
if plot_ndist:
    ...
    figure()
    #n, bins, patches = plt.hist(np.log10(n_array)-np.log10(np.percentile(n_array,50)), n_array, normed=False,histtype='step', cumulative=True,label='all')
    ...
    for Mmin,Mmax in itertools.product(mmin,mmax):
        ...
        for i,j,k in itertools.product(range(nx),range(ny),range(nz)):
            ...
            count=count+1
            #print i,j,k
        n, bins, patches = plt.hist(np.log10(Nmass)-np.log10(np.percentile(Nmass,50)), len(Nmass), normed=False,histtype='step', cumulative=True,label=r"$M_{\rm min}=$"+'%.1f'%Mmin+" "+r"$M_{\rm max}=$"+'%.1f'%Mmax)
...
acf_file='../data/obs/ACF/Bielby2015.txt'
trans_file='../data/obs/nb497trans_mpch.txt'
theta,corrobs,dcorrobs=np.genfromtxt(acf_file,unpack=1)
theta=theta*60.0
theta=np.log10(theta)

posz,trans=genfromtxt(trans_file,unpack=1)
trans_func = interp1d(posz, trans, kind='linear')
...
def corelationlike(Mmin,dM,focc,x,y,z,mass,index,n_array,xwidth,ywidth,zwidth,Dc=Dc,Da=Da,Dl=Dl,cat_number=1,logtheta=1,convert_to_arcsec=1,estimator='landy',acf_file='../data/obs/ACF/Bielby2015.txt',Nlae=Nlae_mean,RR1=RR,return_corr=0,trans=use_trans,trans_file='../data/obs/nb497trans_mpch.txt',cont_frac=fc):
    ...
    if Mmin>=Mmax-0.00001 or focc>1 or focc<0 or Mmin<9.19 or Mmax<9.24 or  Mmax>13.4 or Mmin>12.5: chi2=np.inf; print chi2; return -1.0*np.inf
        ...
        x1=x[index[count]];y1=y[index[count]];z1=z[index[count]];m1=m[index[count]]
        
        wmin=m1>Mmin; wmax=m1<Mmax
        wm=wmin*wmax
        #print x1, y1, wm
        #raw_input("Press Enter to continue...")
        xm=x1[wm]; ym=y1[wm]; zm=z1[wm]; n_points1=len(xm); n_points=len(xm)
        ...
        zapp=zm[wz]
        transm_amp=0.8
        Nhalo=transm_amp*len(zapp)*mean(n_array)/n_array[0] # a crude stimation of the mean number of halos with Mmin<m<Mmax in a box
        focch=Nlae/Nhalo
        Nlae_teo=focc*Nhalo
        dNlae=np.sqrt(Nlae)
        ...
        if  (1.0*Nlae/Nlae_teo)>6.3 or (1.0*Nlae_teo/Nlae)>6.3:
            ...
            for i,j,k in itertools.product(range(nx),range(ny),range(nz)):
                if trans:
                    ...
                    x1=x[index[count]];y1=y[index[count]];m1=m[index[count]];z1=z[index[count]]
                    wmin=m1>Mmin; wmax=m1<Mmax
                    wm=wmin*wmax
                    xm=x1[wm]; ym=y1[wm]; zm=z1[wm]; n_points1=len(xm);n_points=len(xm)
                    zcenter=(z1.max()+z1.min())/2.0 - zcentroid 
                ...
                if Nlae<=n_points:
                    if trans:
                        for iter in range(4):
                            rpos=rejectionsample(n_points1,Nlae,zm,trans_func)
                            rpos=np.array([int(r) for r in rpos])
                            n_points=Nlae
                            xm1=xm[rpos];ym1=ym[rpos]
                            if cont_frac!=0:
                                ncont=np.int(cont_frac*n_points)
                                xc= xwidth*(i+np.random.random_sample(np.int(ncont)))
                                yc= ywidth*(j+np.random.random_sample(np.int(ncont)))
                                rcont=randsample(n_points,ncont)
                                xm1[rcont]=xc
                                ym1[rcont]=yc

                        
                            DD1,bins=DD_histogram(xm1,ym1,Dc,th_min,th_max,theta_bins,logtheta=logtheta)
                            DD=DD+DD1
                            xr= xwidth*(i+np.random.random_sample(n_points*cat_number))
                            yr= ywidth*(j+np.random.random_sample(n_points*cat_number))
                            DR1,bins=DR_histogram(xm1,ym1,xr,yr,Dc,th_min,th_max,theta_bins,cat_number=cat_number,logtheta=logtheta)
                            DR=DR+DR1
