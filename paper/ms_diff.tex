...
%DIF DEL ms_old.tex   Tue Jun 14 10:58:46 2016
%DIF ADD ms.tex       Tue Jun 14 12:40:34 2016
...
%		FRONT MATTER
%=========================================================================
\title{\DIFdelbegin \DIFdel{Uncertainty }\DIFdelend \DIFaddbegin \DIFadd{Impact of cosmic variance }\DIFaddend on the galaxy-halo connection for Lyman-$\alpha$ emitters \DIFdelbegin \DIFdel{at $z=3.1$}\DIFdelend }
\author{
...
\begin{abstract}
 \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In this paper we }\DIFaddend study the impact of cosmic variance and observational uncertainties
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{To this end, we }\DIFaddend construct mock catalogs from an N-body simulation to match the 
...
We \DIFaddbegin \DIFadd{proceed to  }\DIFaddend explore the parameter space determined by \mmin, \mmax\ and \focc\
with a Markov Chain Monte-Carlo algorithm using \DIFdelbegin \DIFdel{as observational
constraints }\DIFdelend the angular correlation function (ACF) and the LAEs number density  \DIFaddbegin \DIFadd{as observational constraints}\DIFaddend . 
...
As a consequence the median mass, $M_{50}$, of all \DIFdelbegin \DIFdel{models consistent with
observations }\DIFdelend \DIFaddbegin \DIFadd{the consistent models }\DIFaddend has a large uncertainty $M_{50} =
...
We are also able \DIFaddbegin \DIFadd{to }\DIFaddend show that \focc\ is uniquely determined by $M_{\rm
...
distribution width \DIFdelbegin \DIFdel{, $W_{1\sigma}$, }\DIFdelend constructed over several fields of
$\sim 1$ deg$^{2}$.
\DIFaddbegin \DIFadd{These results suggest that taking explicitly into account the effect of cosmic
variance using cosmological simulations is a valuable tool to 
interpret LAEs observations at high redshift.
...
the conclusions derived on the halo host mass do not elaborate \DIFaddbegin \DIFadd{deeply }\DIFaddend on the
uncertainty resulting from the cosmic variance on these fields. 
\DIFaddbegin \DIFadd{For instance, \mbox{%DIFAUXCMD
\citet{Guaita2010} }%DIFAUXCMD
attempted to analytically 
estimate the uncertainty from cosmic variance 
following \mbox{%DIFAUXCMD
\citet{Somerville2004} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Peebles1980}}%DIFAUXCMD
. 
Their prescription assumed that the correlation function can be
represented by a power-law
$\xi\left(r\right)=\left(r_{0}/r\right)^{\gamma}$. 
...
\DIFaddbegin \DIFadd{This approach allows us to estimate cosmic variance directly from
...
at $z\sim 3$  with Ly-$\alpha$ \DIFdelbegin \DIFdel{equivalent widths of }\DIFdelend \DIFaddbegin \DIFadd{rest-frame equivalent widths
}\DIFaddend $\gtrsim$65\AA\ and a  Ly-$\alpha$ flux limit of $2\times10^{17}{\rm
  erg/cm^2/s}$ ($L\sim 7\times10^{42}{\rm erg/s} $).  
...
\DIFadd{We chose this simulation based on three requirements. 
...
down to  $10^{10}$}\hMsun\DIFadd{, which is the lower halo mass bound suggested
by previous observational 
studies \mbox{%DIFAUXCMD
\citep{Hayashino2004,Gawiser2007,Ouchi2010,Bielby16}}%DIFAUXCMD
. 
Third, the total volume of the simulation is at least 10 times
larger than the equivalent volume of a single LAE observational field.
These conditions are met as we describe below. 
}

\DIFaddend The cosmological parameters are consistent with Planck
...
algorithm. 
\DIFdelbegin \DIFdel{The catalogs were obtained from the publicly available Multidark
database  }\footnote{%DIFDELCMD < \url{http://www.multidark.org/MultiDark/}%%%
}
%DIFAUXCMD
...
\citep{MultiDark}}%DIFAUXCMD
. 
}\DIFdelend For each  halo in the box we extract its comoving position and mass.  
...
\caption{One and two dimensional projections of the posterior
  ...
  \DIFaddbeginFL \DIFaddFL{Vertical lines in the one dimensional proyections correspond to the
  14-50,84 percentiles of the distribtions.
  Thick lines in the two dimensional planes mark the 1-2-3 $\sigma$
  contours of the corresponding joint distributions.
  }\DIFaddendFL The models with $\log \focc>0.00$ ($\focc>1$) correspond  to models
...
We \DIFaddbegin \DIFadd{do not include the covariance matrix due to the
practical impossibility to estimate it in a robust way.
The small size of the fields and galaxies make it
unfeasible the application of jackknife and bootstrap techniques \mbox{%DIFAUXCMD
\citep{2009MNRAS.396...19N}}%DIFAUXCMD
. 
...
on clustering analyses \mbox{%DIFAUXCMD
\citep{2011ApJ...736...59Z}}%DIFAUXCMD
. 
}



\DIFadd{We }\DIFaddend compute the $\rm{ACF}_{\mathcal M}$ using the Landy \&  Szalay
...
\focc\ is completely determined by \mmin\ from \focc$=$\DIFdelbegin \DIFdel{0.011 }\DIFdelend \DIFaddbegin \DIFadd{0.004 }\DIFaddend when
$\log\mmin=10.0$ to \DIFdelbegin \DIFdel{$\focc=0.39$ }\DIFdelend \DIFaddbegin \DIFadd{$\focc=0.38$ }\DIFaddend when $\log\mmin=11.15$. 
We compute the power-law dependence between \focc\ and \mmin\ to be 
\begin{equation}
\focc = \DIFdelbegin \DIFdel{0.08}\DIFdelend \DIFaddbegin \DIFadd{0.05}\DIFaddend \left(\DIFdelbegin \DIFdel{\frac{\mmin}{10^{10.3}\hMsun}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\mmin}{10^{10}\hMsun}}\DIFaddend \right)^{0.77}.
\DIFaddbegin \label{eq:focc}
...
\DIFaddbegin \DIFadd{The result in Eq.(\ref{eq:focc}) can be qualitatively understood as
follows.   
A choice of $\focc<1$ means that the halo mass function is scaled down.
If }\mmin\DIFadd{\ decreases the total number of halos increases because less
...
kind. 
 ...
 \DIFaddbegin \DIFadd{To keep the validity of Eq. (\ref{eq:width}) there should be of the order of
 $27$ field or even a larger number. Each of these fields should have  $\sim1{\rm
  deg^2}$ in size and reproduce the same observational conditions of 
  \mbox{%DIFAUXCMD
\citet{Bielby16} }%DIFAUXCMD
in terms of filters, equivalent
width cuts .  However, repeating the same kind of cosmic variance 
study for different survey strategies should help to provide a equivalent
constraint. 
}

\subsection{\DIFadd{Hints of larger uncertainties}}

\DIFadd{Fig. \ref{fig:like} shows an interesting feature for }\mmin\DIFadd{. 
The histogram (upper left) showing the probability for this parameter
given the observational constraints does not considerably decreases for lower
masses. 
This raises the following question. Down to which values of
}\mmin\DIFadd{\ does the probability significantly decreases? 
}

\DIFadd{Lower values of }\mmin\DIFadd{\ would imply a lower }\focc\DIFadd{\ and larger
uncertainties on the parameters }\mmin\DIFadd{\, }\focc \DIFadd{and $M_{50}$.
In other words, the limitations in our priors for }\mmin\DIFadd{\ (which
set the limitations for the kind of numerical simulation we use) might
be underestimating the uncertainties and slightly biasing our results
for }\mmin\DIFadd{\, }\focc\DIFadd{\ and $M_{50}$.
}

\DIFadd{Although Fig. \ref{fig:mmed} suggest that models with lower values of
$M_{\rm 50}$, i.e. lower values of }\mmin\DIFadd{, have a higher
$\chi^2_{\mathcal{M}}$ and could be thus discarded, we suggest that a
proper test of this hypothesis requires performing new cosmological
simulations to be able to probe }\mmin\DIFadd{\ masses below our limit of
$6\times  10^{9}$}\hMsun\DIFadd{. 
}

\DIFadd{This highlights the main thesis of this paper. 
Namely, the high impact that cosmic variance can have on constraining
the paremeters of our model. 
With the current limitations in the cosmological simulations available
to the public we cannot significantly decrease or prior on }\mmin\DIFadd{.
We consider such test beyond the scope of this paper.
}

\DIFadd{This extension to larger priors cannot change other central results of
our paper, such as the relationship between }\focc\DIFadd{\ and }\mmin\DIFadd{\, the
narrow mass range for individual models and the possibility to
constraint the halo mass with cosmic variance on the number density
counts. 
However, we suggest that future work building upon the
technique presented in this paper should count with simulations that
extend at least one order of magnitude below our limit $6\times
10^{9}$}\hMsun\DIFadd{.  
}
...
\DIFaddbegin \DIFadd{This work also highlights the need to explore a wider range on the
prior of }\mmin\DIFadd{\ if one wants to better estimate the uncertainty on
}\mmin\DIFadd{, }\focc\DIFadd{\ and $M_{50}$.  
In this paper we use priors given by previous observations, but our
results suggest that once cosmic variance is explicitly taken into
account, even lower values of }\mmin\DIFadd{\ might be in fact consistent with
observations. 
We recommend that future work making use of the technique presented
in this paper should extend the prior for }\mmin\DIFadd{\ to masses at least
one order of magnitude below $6\times 10^{9}$}\hMsun\DIFadd{.  
}

\DIFaddend Nevertheless, our analysis allowed us to draw two results that can
...
\DIFaddbegin \DIFadd{The authors acknowldge the referee, Nelson Padilla, for useful
remarks and questions that helped us to improve the quality and
clarity of our paper. The authors also acknowldge Valentino Gonzalez,
Eric Gawiser and Mark Dijkstra for the fruitful discussion and
suggestions that also contributed to improve the quality of this work. 
}
...
\DIFaddbegin \bibitem[{{Guaita} {et~al.}(2010){Guaita}, {Gawiser}, {Padilla}, {Francke},
  {Bond}, {Gronwall}, {Ciardullo}, {Feldmeier}, {Sinawa}, {Blanc}, \&
  {Virani}}]{Guaita2010}
{\DIFadd{Guaita}}\DIFadd{, L., }{\DIFadd{Gawiser}}\DIFadd{, E., }{\DIFadd{Padilla}}\DIFadd{, N., }{\DIFadd{Francke}}\DIFadd{, H., }{\DIFadd{Bond}}\DIFadd{, N.~A.,
  }{\DIFadd{Gronwall}}\DIFadd{, C., }{\DIFadd{Ciardullo}}\DIFadd{, R., }{\DIFadd{Feldmeier}}\DIFadd{, J.~J., }{\DIFadd{Sinawa}}\DIFadd{, S., }{\DIFadd{Blanc}}\DIFadd{,
  G.~A., \& }{\DIFadd{Virani}}\DIFadd{, S. 2010, }\apj\DIFadd{, 714, 255
}

\DIFaddend \bibitem[{{Hayashino} {et~al.}(2004){Hayashino}, {Matsuda}, {Tamura},
...
{Jose}, C., {Srianand}, R., \& {Subramanian}, K. 2013, \DIFdelbegin \DIFdel{ArXiv e-prints
}\DIFdelend \DIFaddbegin \mnras\DIFadd{, 435, 368
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \bibitem[{{Klypin} {et~al.}(2014){Klypin}, {Yepes}, {Gottlober}, {Prada}, \&
%DIFDELCMD <   {Hess}}]{BolshoiP}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[{{Klypin} {et~al.}(2016){Klypin}, {Yepes}, {Gottl{\"o}ber}, {Prada},
  \& {He{\ss}}}]{BolshoiP}
\DIFaddend {Klypin}, A., {Yepes}, G., {\DIFdelbegin \DIFdel{Gottlober}\DIFdelend \DIFaddbegin \DIFadd{Gottl}{\DIFadd{\"o}\DIFaddend }\DIFaddbegin \DIFadd{ber}}\DIFaddend , S., {Prada}, F., \& {\DIFdelbegin \DIFdel{Hess}\DIFdelend \DIFaddbegin \DIFadd{He}{\DIFadd{\ss}\DIFaddend }\DIFaddbegin }\DIFaddend , S.
  \DIFdelbegin \DIFdel{2014, ArXiv e-prints
}\DIFdelend \DIFaddbegin \DIFadd{2016, }\mnras\DIFadd{, 457, 4340
}\DIFaddend 
...
\DIFaddbegin \bibitem[{{Norberg} {et~al.}(2009){Norberg}, {Baugh}, {Gazta{\~n}aga}, \&
  {Croton}}]{2009MNRAS.396...19N}
{\DIFadd{Norberg}}\DIFadd{, P., }{\DIFadd{Baugh}}\DIFadd{, C.~M., }{\DIFadd{Gazta}{\DIFadd{\~n}}\DIFadd{aga}}\DIFadd{, E., \& }{\DIFadd{Croton}}\DIFadd{, D.~J. 2009,
  }\mnras\DIFadd{, 396, 19
}

\DIFaddend \bibitem[{{Orsi} {et~al.}(2012){Orsi}, {Lacey}, \& {Baugh}}]{Orsi2012}
...
\DIFaddbegin \bibitem[{{Somerville} {et~al.}(2004){Somerville}, {Lee}, {Ferguson},
  {Gardner}, {Moustakas}, \& {Giavalisco}}]{Somerville2004}
{\DIFadd{Somerville}}\DIFadd{, R.~S., }{\DIFadd{Lee}}\DIFadd{, K., }{\DIFadd{Ferguson}}\DIFadd{, H.~C., }{\DIFadd{Gardner}}\DIFadd{, J.~P.,
  }{\DIFadd{Moustakas}}\DIFadd{, L.~A., \& }{\DIFadd{Giavalisco}}\DIFadd{, M. 2004, }\apjl\DIFadd{, 600, L171
}

\DIFaddend \bibitem[{{Springel} {et~al.}(2005){Springel}, {White}, {Jenkins}, {Frenk},
...
\bibitem[{{Zehavi} {et~al.}(2011){Zehavi}, {Zheng}, {Weinberg}, {Blanton},
  {Bahcall}, {Berlind}, {Brinkmann}, {Frieman}, {Gunn}, {Lupton}, {Nichol},
  {Percival}, {Schneider}, {Skibba}, {Strauss}, {Tegmark}, \&
  {York}}]{2011ApJ...736...59Z}
{\DIFadd{Zehavi}}\DIFadd{, I., }{\DIFadd{Zheng}}\DIFadd{, Z., }{\DIFadd{Weinberg}}\DIFadd{, D.~H., }{\DIFadd{Blanton}}\DIFadd{, M.~R., }{\DIFadd{Bahcall}}\DIFadd{,
  N.~A., }{\DIFadd{Berlind}}\DIFadd{, A.~A., }{\DIFadd{Brinkmann}}\DIFadd{, J., }{\DIFadd{Frieman}}\DIFadd{, J.~A., }{\DIFadd{Gunn}}\DIFadd{, J.~E.,
  }{\DIFadd{Lupton}}\DIFadd{, R.~H., }{\DIFadd{Nichol}}\DIFadd{, R.~C., }{\DIFadd{Percival}}\DIFadd{, W.~J., }{\DIFadd{Schneider}}\DIFadd{, D.~P.,
  }{\DIFadd{Skibba}}\DIFadd{, R.~A., }{\DIFadd{Strauss}}\DIFadd{, M.~A., }{\DIFadd{Tegmark}}\DIFadd{, M., \& }{\DIFadd{York}}\DIFadd{, D.~G. 2011,
  }\apj\DIFadd{, 736, 59
}\DIFaddend 
