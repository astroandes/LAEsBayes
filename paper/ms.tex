...
$10^{9.5}$\hMsun\ and $10^{11.5}$\hMsun\ (a $2.0$ dex range), 
...
distribution with as \lmmin\ increases.  This trend is confirmed in
...
observational data can only put weak constraints on \ \mmax\ and \focc. 
When we translate these loos constraints into the median mass, we find
a general consistency with previous works \citep{Hayashino2004,
  Gawiser07,Ouchi2008} specially with the most recent one
\citep{Bielby16}. 

Nevertheless, our analysis allowed us to draw three results that can
be used to put tighter constraints on \mmin, \mmax and \focc once
...
\item \focc is uniquely determined by \mmin, regardless of
  \mmax. A precise determination of \mmin\ will thus fix \focc. As a consequence of this, for a given  \mmin the media mass is also  fixed. 

\item ALSO MENTION THIS HERE BECAUSE IS A STRONG CONSTRAIN:
 the 1-$\sigma$ width of the mass distribution is highly constrained to be between $0.2$ and $0.6$ dex. SOMETHING THAT IS ALSO IMPORTANT IS THAT THIS CONSTRAIN IS ASSUMING OUR MODEL IS RIGHT, IF IT HAPPENS TO BE THAT THE FUNCTION THAT RELATES LUMINOSITY WITH MASS IS VERY STEPPEN THEN THE RESULTS MAY CHANGE.

\item The width of the LAE number distribution function
  obtained over several fields of $\approx 1$ deg$^2$ is tightly
  correlated with \mmin. That measurement with next generation surveys
  will be able to constrain \mmin within a factor of $\sim 2$.
\item The ACF becomes slightly stronger at short angular
  distances ($<1\,\rm arcmin$) when \mmax\ increases therefore precise
 measurements of the  ACF at such scales are crucial to accurately
 determine \mmax.  
\end{enumerate}
