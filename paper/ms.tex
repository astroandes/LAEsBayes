...
However, the same Figure \ref{fig:mmed} shows something interesting
that semi-analytical approaches were not able to predict.
The mass width, $\Delta M{1\sigma}$, that is the width of the mass
distribution for a given model with fixed $\mmin$ and $\mmax$, has a
median value and $1\sigma$ uncertainty of $\Delta M{1\sigma} =
...
In summary, the median mass could be anything in the range
$10^{9.5}$\hMsun and $10^{11.5}$, but the width is highly constrained
