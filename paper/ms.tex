...
$1\sigma$ mass gap $\Delta M_{1\sigma} = 0.55^{+0.11}_{-0.31}$ dex.
...
We focus our work on halos more massive than $6.3\times
10^{9}$\hMsun\ resolved with at least  $40$ particles 
to guarantee a well behaved halo mass function. 
...
The preferred $1-\sigma$ range for the masses is $10.0<\log \mmin<11.1$
and $11.0<\log\mmax<13.0$.
\focc\ is completely determined by \mmin\ from \focc$=$0.015 when
$\log\mmin=9.8$ to $\focc=0.30$ when $\log\mmin=11.0$. 
...
previous estimates reported by
 \citet{Bielby16} ($\tilde{M}_{\mathrm h} = 10^{11.0\pm0.6} \hMsun$),
 \citet{Gawiser07} ($\tilde{M}_{\mathrm h} = 10^{10.9\pm0.9} \hMsun$) and
 \citet{Ouchi2010} ($\tilde{M}_{\mathrm h} = 10^{10.8^{+1.8}_{-0.8}} \hMsun$) 
using semi-analitical approaches.   
In spite of the large uncertainties, we note that our median mass is
systematically below theirs by $0.3$ to $0.5$ dex. 
...
small due to the asymetry in the dark matter halo mass function. 
...
of \lmmin. 
We find that considering all the mock $25$ field the $1\sigma$ width, $W_{1\sigma}$
...
$27$ fields (to follow the same numbers we use here) of size $\sim1{\rm
  deg^2}$ with the same observational coditions (filters, equivalent
width cuts). 
...
\item \focc is uniquely determined by \mmin, regardless of
  ...
  consequence of this, for a given  \mmin the media mass is also
  fixed.  
...
in a cosmological context to better understand why observable LAEs are
