...
if and only if the halo mass is in the range $\mmin < M_{\mathrm
  h} < \mmax$. 
In what follows we refer to a given set of values $\focc$, $\mmin$,
$\mmax$ as a model, $\mathcal{M}$. We also use a variable \dm to represent
the mass range width, $\dm\equiv\ \log \mmax - \log \mmax$.
...
We instead opt for an arithmentic interpretation by setting $\focc$ as
